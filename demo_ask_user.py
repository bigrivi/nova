#!/usr/bin/env python3
"""
Demo script to preview AskUserWizard styles in terminal.

Run: python demo_ask_user.py
Keys: 1/2/3/4 switch scenes, q quit

To use a different theme:
    TEXTUAL_THEME=nord python demo_ask_user.py
    TEXTUAL_THEME=catppuccin-mocha python demo_ask_user.py
"""
...
from nova.cli.widgets import AskUserWizard
...
class DemoApp(App):
    ...
    def action_show_scene(self, key: str) -> None:
        ...
        container.mount(Static(f"[bold cyan]{title}[/]", classes="q-header"))
        wizard = AskUserWizard(questions)
        container.mount(wizard)

    def on_ask_user_wizard_submitted(self, event: AskUserWizard.Submitted) -> None:
        self.notify(f"Submitted: {event.answers}")

    def on_ask_user_wizard_dismissed(self, event: AskUserWizard.Dismissed) -> None:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Demo AskUserWizard with Textual themes")
