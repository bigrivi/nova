/** Nova 后端 HTTP/SSE 客户端（从 frontend/src/lib/nova-api.ts 移植，去掉 Vite 依赖） */
import type {
...
export async function deleteAgent(key: string): Promise<void> {
  const response = await fetch(buildUrl(`/api/agents/${encodeURIComponent(key)}`), {
    method: 'DELETE',
  })
  if (!response.ok) {
    throw new Error(await parseErrorMessage(response))
...
function emitFrame(frame: string, onEvent: (event: NovaStreamEvent) => void): void {
  const payload = frame
    .split('\n')
