/** ask_user 表单弹窗：逐题作答（text/select/confirm），提交答案作为新消息继续流 */
...
export function AskUserForm({ questions }: { questions: AskQuestion[] }) {
  ...
  return (
    ...
    >
      ...
      {question.inputType === 'select' ? (
        <box flexDirection="column">
          {question.options.map((option, index) => (
            <text key={option.label} fg={index === selected ? '#4f9cf9' : '#8b949e'}>
              {index === selected ? '▸ ' : '  '}
              {option.label}
            </text>
          ))}
      ...
      <text fg="#6e7681">
        {step + 1}/{questions.length}  •  {isLast ? 'enter submits' : 'enter next'}
      </text>
    </box>
  )
}
