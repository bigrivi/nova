/** 底部输入区：多行 textarea、Enter 提交、Shift+Enter 换行、斜杠命令建议、ESC 中断 */
import type { KeyBinding, KeyEvent, TextareaRenderable } from '@opentui/core'
import { useEffect, useRef, useState } from 'react'
import { interruptChat } from '../api/nova-api.ts'
import { COMMANDS, matchCommands, parseCommand, type CommandSpec } from '../commands.ts'
...
export function Composer({
  ...
  onCommand: ComposerCommandHandler
}) {
  ...
  function refreshSuggestions(value: string): void {
    ...
    if (ta) {
      ...
      setTextareaHeight((prev) => {
        const next = Math.min(TEXTAREA_MAX_HEIGHT, Math.max(TEXTAREA_MIN_HEIGHT, lines))
  ...
  async function submitCurrent(): Promise<void> {
    ...
    // 命令优先：输入以 / 开头且是已知命令时直接执行（不依赖异步 suggestions state）
    if (value.trim().startsWith('/')) {
      const parsed = parseCommand(value)
      const known = parsed && COMMANDS.some(
        (cmd) => cmd.id === parsed!.id || cmd.aliases.includes(parsed!.id),
      )
      if (parsed && known) {
        clearComposer()
  ...
  return (
    <box
      ...
      borderColor="#444c56"
    >
      <CommandSuggestions items={suggestions} selectedIndex={selectedIndex} />
